...
use models\DesoRetornosModel;
use views\DesoRetornosView;
use \stdClass;

class DesoRetornosController extends Controller
{
	private $model;
	private $view;
	public $result;
	...
	{
		$this->model = new DesoRetornosModel();
		$this->view = new DesoRetornosView();
	...
	public function index()
	{
		...
		$result = $this->model->find("deso_".$this->table, $params, "all", true, $order);
	...
	{
		$result = $this->model->find("deso_".$this->table, [["id", $ID]], "first");
	...
	{
		if (count($_POST) > 0) {
			$save = $this->model->save("deso_".$this->table, $_POST);
			if ($save->success) {
	...
	{
		if (count($_POST) > 0) {
			$save = $this->model->update("deso_".$this->table, $_POST, $ID);
			if ($save->success) {
	...
	{
		$save = $this->model->delete("deso_".$this->table, $ID);
